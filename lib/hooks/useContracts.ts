...
export const useLaunchpadContract = () => {
  ...
  const createLaunch = async (
    ...
    return await writeContractAsync({
      ...
      args: [name, symbol, description, imageUri, BigInt(maxSupply), autoProgress],
    });
  };
...
export const useMarketplaceContract = () => {
  ...
  const listItem = async (
    ...
    return await writeContractAsync({
      ...
      args: [collection, BigInt(tokenId), priceWei, listingType, BigInt(auctionDuration)],
    });
  };

  const buyItem = async (listingId: number, priceInEth: string): Promise<`0x${string}`> => {
    const priceWei = parseEther(priceInEth);
...
export const useNFTCollectionContract = () => {
  ...
  const mint = async (
    ...
    const totalPrice = parseEther((parseFloat(pricePerToken) * amount).toString());
...
export const useCollectionStats = (collectionAddress?: Address) => {
  const nftCollection = collectionAddress || getContractAddress('NFT_COLLECTION');
...
export const usePhaseManagement = (collectionAddress?: Address) => {
  ...
  const configurePhase = async (
    ...
    const hash = await writeContractAsync({
      ...
      args: [phase, parseEther(price), BigInt(startTime), BigInt(endTime), BigInt(maxPerWallet), BigInt(maxSupply)],
    });
    
    return hash;
  };

  const updateWhitelist = async (phase: number, addresses: Address[], status: boolean) => {
    if (!collectionAddress) throw new Error('Collection address required');
  ...
  const mintWithPhase = async (to: Address, phase: number, tokenURI: string, price: string) => {
    if (!collectionAddress) throw new Error('Collection address required');
