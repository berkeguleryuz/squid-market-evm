...
import { useRealUserNFTs, UserNFT } from '@/lib/hooks/useRealUserNFTs';
...
export default function MyNFTsPage() {
  ...
  const { userNFTs, isLoading, error, refetch } = useRealUserNFTs();
  ...
  const totalValue = userNFTs.reduce((sum, nft) => {
    return sum + (nft.price ? parseFloat(nft.price) : 0);
  }, 0);
